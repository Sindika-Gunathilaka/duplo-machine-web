...
class CreateCustomer extends Component {
  ...
  componentDidMount = async () => {
    try {
      ...
      // this.setState({ validateCustomers: customers.content });
    } catch (error) {
      console.log(error);
    }
  ...
  saveButtonHandler = async (event) => {
    ...
    if (validateForm(this.state.errors)) {
      ...
      const customer = await axios.post(BASE_URL + "/v1/customers", postData);
          ...
          <div className={classes.ContentWrapper}>
            ...
            <Row>
              <Col>
                ...
                <select className="form-control" style={{ backgroundColor: "#F7F7F7", borderRadius: " .55rem", cursor: "pointer" }} name={"customer_type"} onChange={(e) => this.onCustomerTypeSelectHandler(e)}>
