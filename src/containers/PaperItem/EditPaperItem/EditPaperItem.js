...
import classes from './EditPaperItem.module.css';
...
var paperCost;
class EditPaperItem extends Component {
  ...
  componentDidMount = async () => {
    let selectedPaperItemId = this.props.selectedPaperItemIDFromRedux.SelectPaperItemId;
    console.log(selectedPaperItemId, "selectedPaperItemId");
    // get paper sizes
    const configs = {
      params: {
          'paperItemName': selectedPaperItemId
      }
  }

    try {
      const getResponse = await axios.get(BASE_URL + "/v1/paper-items/paper_item_name",configs);
      console.log(getResponse,"getResponse");
      await this.setState({
        paper_size_id:getResponse.data.paper_item_id.paper_size_id.id,
        paper_size_name:getResponse.data.paper_item_id.paper_size_id.page_name,
        paper_gsm_id:getResponse.data.paper_item_id.paper_gsm_id.id,
        paper_gsm_name:getResponse.data.paper_item_id.paper_gsm_id.page_gsm,
        papers_per_rim:getResponse.data.papers_per_rim,
        price_of_rim:getResponse.data.price_of_rim,
        pre_paper_gsm_name: getResponse.data.paper_item_id.paper_gsm_id.page_gsm,
        pre_paper_size_name:getResponse.data.paper_item_id.paper_size_id.page_name
      })
    } catch (error) {
      console.log(error);
    }
          ...
          <div className={classes.ContentWrapper}>
            <Row>
              ...
              <Col>
                ...
                <select className="form-control" style={{ backgroundColor: "#F7F7F7", borderRadius: " .55rem", cursor: "pointer" }} name={"paper_size_id"} onChange={(e) => this.onPaperSizeSelectHandler(e)} disabled>
                  <option selected>{"Selected "+this.state.pre_paper_size_name}</option>
              ...
              <Col>
                ...
                <select className="form-control" style={{ backgroundColor: "#F7F7F7", borderRadius: " .55rem", cursor: "pointer" }} name={"paper_gsm_id"} onChange={(e) => this.onPaperGsmSelectHandler(e)} disabled>
                  <option selected>{"Selected "+this.state.pre_paper_gsm_name}</option>
...
const mapSateToProps = (state) => {
  return { selectedPaperItemIDFromRedux: state.editPaperItem }
};

export default connect(mapSateToProps)(EditPaperItem);
